...
export default async function AdminTablePageWithPagination({
  params,
}: {
  params: Promise<{ table: string; page: string }>;
}) {
  ...
  const { table, page: pageParam } = await params;
  const page = parseInt(pageParam, 10);
  const data = await getTableData(table, page);
