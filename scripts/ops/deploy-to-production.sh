...
# 環境設定
# R2_BUCKET="monogs-r2-upload"
D1_DATABASE="monogs-db"
D1_ID="d7781133-52aa-41f4-8a30-af1e6c0934b4"

# echo -e "${YELLOW}Step 1: R2 production バケットをクリア${NC}"
...
echo ""
echo -e "${YELLOW}Step 1: D1データベースをクリアして再構築${NC}"
echo "データベース: $D1_DATABASE"
read -p "D1データベースを初期化しますか？ (y/N): " confirm
if [[ $confirm == [yY] ]]; then
  echo "D1データベースをクリア中..."
  # マイグレーションを削除して再実行（順序を変更し、d1_migrationsも追加）
  npx wrangler d1 execute $D1_DATABASE --remote --command="DROP TABLE IF EXISTS d1_migrations; DROP TABLE IF EXISTS posts_tags; DROP TABLE IF EXISTS posts; DROP TABLE IF EXISTS tags; DROP TABLE IF EXISTS sessions; DROP TABLE IF EXISTS users; DROP TABLE IF EXISTS settings;" || true
...
echo ""
echo -e "${YELLOW}Step 2: SQLiteからD1にデータを移行${NC}"
...
echo ""
echo "Production URL: https://monogs-production.shirabegroup.workers.dev"
