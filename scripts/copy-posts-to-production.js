...
/**
 * Preview環境からProduction環境にpostsデータをコピー
 */

const { execSync } = require('child_process');

// Preview環境からpostsデータを取得
console.log('Fetching posts from production...');
const postsJson = execSync(
  'npx wrangler d1 execute monogs-db-preview --remote --command="SELECT * FROM posts" --json',
  { encoding: 'utf-8' }
);
...
for (let i = 0; i < posts.length; i++) {
  ...
  try {
    execSync(
      `npx wrangler d1 execute monogs-db --remote --command="${insertSql.replace(/"/g, '\\"')}"`,
